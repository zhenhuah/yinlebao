...
$year = array('2014');
foreach($year as $vy){
	$f1 = 'http://tv.video.qq.com/fcgi-bin/dlib/dout_tv?auto_id=16&platform=10&version=10005&itype=-1&iarea=-1&sort=1&pagesize=20&iyear='.$vy.'&page=0';
	echo $f1.'\n';
	$sxmln = afget ( $f1 );
	...
	for($i=0;$i<=$num;$i++){
		$f1 = 'http://tv.video.qq.com/fcgi-bin/dlib/dout_tv?auto_id=16&platform=10&version=10005&itype=-1&iarea=-1&sort=1&pagesize=20&iyear='.$vy.'&page='.$i;
		echo $f1.'\n';
		$sxmly = afget ( $f1 );
...
function doalist($v){
	...
	$go3cdb = yzy_go3c_db() ;
	//先判断是否已经采集过
	$tmp = $phpcmsdb->r1('v9_video',array('title'=>$v['c_title'],'column_id'=>'7','ispackage'=>'1'),'*') ;
	if(!empty($tmp)){   //视频已入库则更新tag等数据
		...
		if(!empty($tgo3c)){
			...
			if($tmp['asset_id']=='yzd10o9e4tekrud'){
				echo $tmp['asset_id'].'\n';
			}else{
				echo $tmp['asset_id'].'\n';
			}
