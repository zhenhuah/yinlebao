...
function doa($a){
	...
	//开始导入电影的播放链接
		...
		//查询此视频链接是否存在
		if(strpos($url,'flv.vodfile.m1905.com')!== false) continue ;
		if(strpos($url,'data1.hdpfans.com:80/xunlei.php')!== false) continue ;
		if(strpos($url,'http://.flv')!== false) continue ;
		if(strpos($url,'vodipad.wasu.cn')!== false) continue ;
		$cd = array(
				'title'   	=> $vi['title'] ,
				'asset_id'  => $vi['asset_id'],
				'path'   	=> $url ,
		...
		$tmp = $phpcmsdb->r1('v9_video_content',array('asset_id'=>$vi['asset_id'],'path'=>$url,'source_id'=>$source_id)) ;
		if($tmp['id']) {
	...
	if(empty($gtmp)){	//视频没有上线
		...
		//视频支持终端控制先清理，再添加
		$go3cdb->d('video_play_control',array('vid'=>$r['asset_id'])) ;
		for($i=1;$i<5;$i++){
			$STB = array(
				'vid' => $vi['asset_id'],
				'term_id' => '$i',
