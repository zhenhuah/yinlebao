...
function doa($a){
	...
	if(strpos($vi['actors'],'鼠标')!== false){
		$vi['ANDROID']='1' ;
		$vi['term_id'] = $vi['term_id'].','.'mouse';
	}
	if(strpos($vi['actors'],'手柄')!== false){
		$vi['ANDROID']='1' ;
		$vi['term_id'] = $vi['term_id'].','.'handle';
	}	
	$sql1="SELECT app_id from app where packagename ='$vi[packagename]'";
	$thisv= $shopdb->rs($sql1) ;
	if(!empty($thisv)){
		$numm = count($thisv);
		echo $numm."....ok!\n";
		foreach($thisv as $v){
			...
			$shopdb->w('app_download_info',$dd,array('app_id'=>$v['app_id'])) ;
			echo $v['app_id']."....ok!!\n";
		}
	}else{
		echo "net one ...\n";
