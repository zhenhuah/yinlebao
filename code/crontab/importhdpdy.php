...
function doa($a){
	...
	$column_id = 5;
	//生成标题首字母
	$zh = preg_replace('((?=[\x21-\x7e]+)[^A-Za-z0-9])', '', $vi['title']);
	$ret = "";
	...
	if($num<=2){
		...
		if(strpos($url,'youku')!== false){
			...
			$source_id=21;
		}elseif(strpos($url,'letv.cn')!== false){
			$source_id=16;
	...
	if(empty($gtmp)){	//视频没有上线
		...
		//增加视频的tag
		$taggo3c = $go3cdb->r1('tag',array('tag_name'=>$a['type_name'])) ;
		if(empty($taggo3c)){
			$tag_id = '3';
		}else{
