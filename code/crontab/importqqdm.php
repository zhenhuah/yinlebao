...
//  http://tv.video.qq.com/fcgi-bin/dlib/dout_tv?auto_id=5&platform=10&version=10005

$year = array('2014','2013','2012','2011','2010','2009','2008','2007','2006','2005','2004','2003','2002');
foreach($year as $vy){
	$f1 = 'http://tv.video.qq.com/fcgi-bin/dlib/dout_tv?auto_id=16&platform=10&version=10005&itype=-1&iarea=-1&itrailer=-1&iedition=-1&sort=1&pagesize=20&iyear='.$vy.'&page=0';
	$sxmln = afget ( $f1 );
	...
	for($i=0;$i<=$num;$i++){
		$f1 = 'http://tv.video.qq.com/fcgi-bin/dlib/dout_tv?auto_id=16&platform=10&version=10005&itype=-1&iarea=-1&itrailer=-1&iedition=-1&sort=1&pagesize=20&iyear='.$vy.'&page='.$i;
		$sxmly = afget ( $f1 );
...
function dov($v){	
	...
	$c_timelong = ereg_replace('[^0-9]','',$v['c_timelong']);
	$rating = $v['rating'];
	$ispackage = 1 ;
	//生成标题首字母
	...
	$d = array(
			...
			'area_id' 	 	=> $tarea['id'],
			'is_free' 	    => '1',
			'total_episodes'=> $c_timelong,
			'parent_id' 	=> '',
			'episode_number' => '',
			'latest_episode_num' => $c_timelong ,
			'rating'      	=> $rating ,
			'ispackage'   	=> $ispackage ,
			'catid'   	 	=> '54',
			'status'     	=> '99',
	...
	//视频树图图片入库
	$picty = array('1'=>'102','2'=>'202','3'=>'302','4'=>'402');
