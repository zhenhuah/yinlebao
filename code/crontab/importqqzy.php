...
foreach($year as $vy){
	$f1 = 'http://tv.video.qq.com/fcgi-bin/dlib/dout_tv?auto_id=17&platform=10&version=10005&itype=-1&iarea=-1&itrailer=-1&iedition=-1&sort=1&pagesize=20&iyear='.$vy.'&page=0';
	$sxmln = afget ( $f1 );
	...
	for($i=0;$i<=$num;$i++){
		$f1 = 'http://tv.video.qq.com/fcgi-bin/dlib/dout_tv?auto_id=17&platform=10&version=10005&itype=-1&iarea=-1&itrailer=-1&iedition=-1&sort=1&pagesize=20&iyear='.$vy.'&page='.$i;
		$sxmly = afget ( $f1 );
...
function dov($v){	
	...
	$c_variety_date = $v['c_timelong'];
	$c_variety_date = ereg_replace('[^0-9]','',$v['c_variety_date']);
	$rating = $v['rating'];
	$ispackage = 1 ;
	//生成标题首字母
	...
	$d = array(
			...
			'area_id' 	 	=> $tarea['id'],
			'is_free' 	    => '1',
			'total_episodes'=> $c_variety_date,
			'parent_id' 	=> '',
			'episode_number' => '',
			'latest_episode_num' => $c_variety_date ,
			'rating'      	=> $rating ,
			'ispackage'   	=> $ispackage ,
			'catid'   	 	=> '54',
			'status'     	=> '99',
