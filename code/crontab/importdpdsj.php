...
function doalist($alist){
	...
	foreach($alist as $v){
		//先判断是否已经采集过
		$tmp = $phpcmsdb->r1('v9_video',array('title'=>$v['title']),'*') ;
		if(isset($tmp['id'])){
			$v['id'] = $tmp['id'] ;
		}
		//有重复的，那么增加tag
...
function doa($a){
	...
	$thisd = $phpcmsdb->r1('v9_video',array('title'=>$vi['title'],'column_id'=>'4'),'id,published,online_status') ;
