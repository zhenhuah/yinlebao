...
//服务器的图片路径地址
define('TASK_IMG_PATH','http://192.168.19.108:8060/images/go3ccms/');
//define('TASK_IMG_PATH','http://go3c.jdjkcn.net/');

//本地的客户端的图片域名(项目域名)(同步的时候用)
define('TASK_IMG_PATH_Local','http://192.168.19.108:8060/go3ccms/');
...
$phpcmsdb = yzy_phpcms_db() ;
   ...
   if (isset($_POST["PHPSESSID"])) {
	...
	$qhjsw=date('YmdHis');
	$imgpath="uploadfile/image/";  //图片保存的路径,其格式必须如此
	...
	if(is_dir($imgpath.$dir_file)){ 
		...
		if(!move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$fileName)) {
			...
			exit(0);
				...
				$path = $host.$rootfoldername."/".$imgpath.$dir_file."/".$fileName;
				$img_path = $imgpath.$dir_file."/".$fileName;	//路径
				...
				$dd['width'] = $width;
				$dd['height'] = $height;
				$dd['asset_id'] = $asset_id;
				$phpcmsdb->w('v9_pic',$dd) ;
