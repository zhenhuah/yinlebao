...
class import_admin extends admin {
	private $db;
	...
	public function init() {
		...
		$list = $this->db->listinfo(array('id'=>$this->get_id()), '', $page, 20);
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=import&c=import_admin&a=add\', title:\''.L('added_external_data_source').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('added_external_data_source'));
		include $this->admin_tpl('import_list');
	}
	
	public function add() {
		if (isset($_POST['dosubmit'])) {
			$name = isset($_POST['name']) && trim($_POST['name']) ? trim($_POST['name']) : showmessage(L('import_name').L('empty'));
			...
			//检察数据源名是否已经存在
			if ($this->db->get_one(array('id'=>$id, 'name'=>$name), 'id')) {
				showmessage(L('import_name').L('exists'));
			}
			
			if ($this->db->insert(array('id'=>$id, 'name'=>$name,'host'=>$host,'port'=>$port,'username'=>$username,'password'=>$password,'dbname'=>$dbname,'dbtablepre'=>$dbtablepre,'charset'=>$charset))) {
				import_cache();
				showmessage('', '', '', 'add');
			} else {
			...
			include $this->admin_tpl('import_add');
		}
	}
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$charset = isset($_POST['charset']) && in_array(trim($_POST['charset']), array('gbk','utf8', 'gb2312', 'latin1')) ? trim($_POST['charset']) : showmessage(L('charset').L('illegal_parameters'));
			$id = $this->get_id();
			$sql = array('id'=>$id, 'host'=>$host,'port'=>$port,'username'=>$username,'password'=>$password,'dbname'=>$dbname, 'dbtablepre'=>$dbtablepre, 'charset'=>$charset);
			...
			pc_base::load_sys_class('form', '', 0);
			$show_header = $show_validator = true;
			include $this->admin_tpl('import_edit');
		}
	}
	...
	public function del() {
		...
		if ($this->db->get_one(array('id'=>$id))) {
			if ($this->db->delete(array('id'=>$id))) {
				import_cache();
				showmessage(L('operation_success'), '?m=import&c=import_admin&a=init');
			} else {
				showmessage(L('operation_failure'),  '?m=import&c=import_admin&a=init');
			}
		} else {
			showmessage(L('notfound'),   '?m=import&c=import_admin&a=init');
		}
	}
