			...
			if($category['type']==0) {
				...
				if(isset($_POST['dosubmit'])) {
					...
					if($catid == '63'){
						...
						//删除已存在的记录
						//$this->tv_channel_area_mapping_model->delete(array('channel_id'=>$channel_id));
						...
						foreach ($menuid as $vc){
							...
							//循环插入区域记录
							$this->tv_channel_area_mapping_model->insert($data_ch_a, true);
						}
						//增加写入支持的终端video表
						$STB = empty($_POST['STB'])?0:$_POST['STB'];
						$PAD = empty($_POST['PAD'])?0:$_POST['PAD'];
	...
	public function edit() {
			...
			if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
				...
				if($catid == '63'){
					...
					$menu->icon = array('│ ','├─ ','└─ ');
					$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
					$channel_area = $this->tv_channel_area_mapping_model->select(array('channel_id'=>$limitInfo['channel_id'])); //频道支持的区域
					//根据管理员权限筛选区域
					$this->admin_model = pc_base::load_model('admin_model');
					$userid = $_SESSION['userid'];
					$ad_area = $this->admin_model->get_one(array('userid'=>$userid));
