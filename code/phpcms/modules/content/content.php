	...
	public function add() {
		...
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($category['type']==0) {
				...
				if($catid == '54'){
					//NJPHP 增加对于发行年代的判断，不能大于当前年份
					$year_released = trim($_POST['info']['year_released']);
					if( $year_released > date('Y',time()) ){
						showmessage('发行日期只能为4为数字年份,且不能大于当前年份', HTTP_REFERER);
					}
					//判断分集视频id和总集id不能为一样
					$parent_id = trim($_POST['info']['parent_id']);
					$asset_id = trim($_POST['info']['asset_id']);
					if($parent_id==$asset_id){
						showmessage('视频id填写错误,请检查!', HTTP_REFERER);
					}
				}
				if ($catid == '68') {
					$ad = $_POST['info'];
					$this->adverts_db = pc_base::load_model('cms_adverts_model');
					$cidStr = $ad['cidStr'];
					$cidArr = explode(',', $cidStr);
					if ($ad['term_id'] == 1)
						$term = 'STB';
					else if ($ad['term_id'] == 2)
					    $term = 'PAD'; 
					else if ($ad['term_id'] == 3)
					    $term = 'PHONE'; 
					else if ($ad['term_id'] == 4)
					    $term = 'PC'; 
					foreach ($cidArr as $v) {
						if ($ad['ad_belong'] == 2) {
							//开机资源类型不能重复添加
							$res = $this->adverts_db->get_one(array('ad_belong'=>2,'spid'=>$v,'genus'=>$ad['title']));
							if(!empty($res))
						...
						'term_id' => $ad['term_id'],
						'type_id' => $ad['type_id'],
						'ad_belong' => $ad['ad_belong'],
						'genus' => $ad['title'],
						'board_type' => $ad['board_type'],
						'display_type' => $ad['display_type']
						);
						$this->adverts_db->insert($data);
					}
					showmessage('操作成功','?m=go3c&c=adverts&a=adverts_list');
				}
				if($catid == '63'){
					$channel_id = $_POST['info']['channel_id'] = intval($_POST['info']['channel_id']);
					$category=$_POST['channel_category'];//一个数组；
					$category=implode(",", $category);//一个字符串；
					if(empty($category)){
							showmessage('频道类型不能为空!', HTTP_REFERER,$ms = 600);
					}
					$menuid=$_POST['menuid'];//一个数组;
						if(empty($menuid)){
							showmessage('频道区域不能为空!', HTTP_REFERER,$ms = 1000);
						}
				}
				//插入文章
				...
				if($catid == '54'){
					...
					//NJPHP Start 增加写入SPID
					$this->video_db = pc_base::load_model('cms_video_model');
					$this->video_db->update(array('spid'=>$this->current_spid[spid]), array('id'=>$content_id));
					//NJPHP Start 如果是电视节目并且有parent_id,自动帮插入海报
					if(isset($_POST['info']['parent_id'])){
						//根据parent_id对应的asset_id查出所有的海报
						$this->poster_db = pc_base::load_model('cms_video_poster_model');
						$poster_list = $this->poster_db->select(array('asset_id'=>$_POST['info']['parent_id']));
						//将查询出来的 $poster_list 信息插入到 poster 表，按照新的 $content_id 插入
						foreach ($poster_list as $value) {
							$new_title = str_replace($value['asset_id'], $_POST['info']['asset_id'], $value['title']);
							$data_poster = array(
								'catid'      => '65',
								'title'      => $new_title,
								'status'     => '99',
								...
								'path'   	 => $value['path'],
								'type'   	 => $value['type'],
								'asset_id'   => $_POST['info']['asset_id'],
							);
							//echo '<pre>';print_r($data_poster);
							$this->poster_db->insert($data_poster, true);
						}
					}
				}
				//exit;
				...
				if($catid == '63'){
					...
					$this->db_channel = pc_base::load_model('cms_channel_model');
				}	
				if(isset($_POST['dosubmit'])) {
					if($catid == '54'){
						//增加写入支持的终端video表
						$STB = empty($_POST['STB'])?0:$_POST['STB'];
						$PAD = empty($_POST['PAD'])?0:$_POST['PAD'];
						$PHONE = empty($_POST['PHONE'])?0:$_POST['PHONE'];
						$PC = empty($_POST['PC'])?0:$_POST['PC'];
						$d_pc = array(
								'STB'=>$STB,
								'PAD'=>$PAD,
								'PHONE'=>$PHONE,
						...
						$this->video_db->update($d_pc, array('asset_id'=>$asset_id));
						//写入点播视频区域数据
						...
						foreach ($menuid as $vc){
							...
							//循环插入点播区域记录
							$this->video_area->insert($data_vi, true);
						}
					}
					if($catid == '63'){
						$this->cms_channel_area_model = pc_base::load_model('cms_channel_area_model');
						$channel_id = $_POST['info']['channel_id'];
						//删除已存在的记录
						$this->cms_channel_area_model->delete(array('channel_id'=>$channel_id));
						$this->cms_channel = pc_base::load_model('cms_channel_model');
						//$aKey = "channel_id = '".$channel_id."'";
						//$limitInfo = $this->cms_channel->get_one($aKey);
						$menuid=$_POST['menuid'];//一个数组;
						if(empty($menuid)){
							showmessage('频道区域不能为空!', HTTP_REFERER,$ms = 1000);
						}
						foreach ($menuid as $vc){
							...
							//循环插入区域记录
							$this->cms_channel_area_model->insert($data_ch_a, true);
						}
						//增加写入支持的终端video表
						$STB = empty($_POST['STB'])?0:$_POST['STB'];
						$PAD = empty($_POST['PAD'])?0:$_POST['PAD'];
						$PHONE = empty($_POST['PHONE'])?0:$_POST['PHONE'];
						$PC = empty($_POST['PC'])?0:$_POST['PC'];
						if(empty($_POST['STB'])&&empty($_POST['PAD'])&&empty($_POST['PHONE'])&&empty($_POST['PC'])){
							showmessage('支持终端不能为空!', HTTP_REFERER,$ms = 1000);
						}
						$d_pc = array(
								'STB'=>$STB,
								'PAD'=>$PAD,
						...
						$this->cms_channel->update($d_pc, array('channel_id'=>$channel_id));
					}
					...
					if($catid == '63'){
						$this->db_channel->update(array('channel_category'=>$category), array('channel_id'=>$channel_id));
					}	
					$goback = param::get_cookie('goback');
					if(strlen($goback) > 0)	{	 
						showmessage(L('add_success'),$goback);
					}else{
						showmessage(L('add_success'),'/index.php?m=content&c=content&a=init&menuid=822');
					}
				} else {
			...
			param::set_cookie('goback', base64_decode($_GET['goback']));
			else
			param::set_cookie('goback', '');
	...
	public function edit() {
			...
			if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
				...
				if($catid == '54'){
					...
					$this->video_db->update($d_pc, array('id'=>$id));
					//写入点播视频区域数据
					$this->video_area = pc_base::load_model('cms_video_area_model');
					//过滤记录
					$this->video_area->delete(array('asset_id'=>$asset_id));
				...
				if($catid == '9'){ //智能epg分类
					...
					$column_id = $_POST['info']['column_id'];
					if($column_id=='3'){
						$column_name = '综艺';
					}elseif($column_id=='4'){
						$column_name = '电视剧';
					...
					$this->cms_channelepg->update($d_epg, array('id'=>$id));
				}
				if($catid == '63'){
					$this->cms_channel_area_model = pc_base::load_model('cms_channel_area_model');
					$channel_id = $_POST['info']['channel_id'];
					...
					foreach ($menuid as $vc){
						...
						$data_ch_a = array(
							'channel_id'=>$channel_id,
							'area_id'=>$area_id,
							'category_id'=>$limitInfo['channel_category'],
							'seq_number'=>$limitInfo['listorder'],
							'isextend'=>'0'
						);
						//循环插入区域记录
						$this->cms_channel_area_model->insert($data_ch_a, true);
				...
				if(isset($_POST['dosubmit'])) {
					...
					//NJPHP	
					if(isset($_POST['doaftersave']) && strlen($_POST['doaftersave']) > 1 ){
						showmessage(L('update_success'),$_POST['doaftersave'], $ms = 60);
						header("Cache-control: private");
						return;	
					}
					$goback = param::get_cookie('goback');
					if(strlen($goback) > 0)		 
					showmessage(L('update_success'),$goback);
					else
					showmessage(L('update_success'),'?m=content&c=content&a=init&catid='.$catid.'&pc_hash='.$_SESSION['pc_hash']);
				...
				if($catid == '54'){
					...
					$this->dbmover_source_type = pc_base::load_model('cms_video_source_model');
					$this->cms_video_subtitle = pc_base::load_model('cms_video_subtitle_model');
					$this->cms_video   = pc_base::load_model('cms_video_model');
					
					$rv = $this->cms_video->get_one(array('id'=>$id));

					$cms_video_content   = $this->cms_video_content->select(array('asset_id'=>$r['asset_id']));
					$cms_video_poster 	 = $this->cms_video_poster->select(array('asset_id'=>$r['asset_id']));
					$dbmover_link_type   = $this->dbmover_link_type->select();
					$dbmover_poster_type = $this->dbmover_poster_type->select();
					$dbmover_source_type = $this->dbmover_source_type->select();
					$video_subtitle  = $this->cms_video_subtitle->select(array('asset_id'=>$rv['asset_id']));
					
					foreach ($dbmover_link_type as $value) {
						$dbmover_link_type_array[$value['id']] = $value;
					}
					
					foreach ($dbmover_poster_type as $value) {
						$dbmover_poster_type_array[$value['id']] = $value;
					}
					
					foreach ($dbmover_source_type as $value) {
						$dbmover_source_type_array[$value['id']] = $value;
					}
					//修改点播区域
					$this->cms_area = pc_base::load_model('cms_area_model');
					$this->video_area = pc_base::load_model('cms_video_area_model');
					...
					$menu->icon = array('│ ','├─ ','└─ ');
					$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
					$videoda = $this->video_area->select(array('asset_id'=>$rv['asset_id']));
					//根据cms管理员权限筛选区域
					$this->admin_model = pc_base::load_model('admin_model');
					$userid = $_SESSION['userid'];
					$ad_area = $this->admin_model->get_one(array('userid'=>$userid));
				...
				if($catid == '63'){
					...
					$cms_channel   = $this->cms_channel->select(array('id'=>$id));
					
					$str = $cms_channel[0]['channel_category'];
					foreach($channel_category as $key=>$sv){
						$att = explode(",",$str);
						foreach($att as $vt){
							if($sv['id']==$vt){
								$channel_category[$key]['kt'] = 1;
							}
						}
					}
					$aKey = "id = '".$id."'";
					...
					$menu->icon = array('│ ','├─ ','└─ ');
					$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
					$channel_area = $this->cms_channel_area_model->select(array('channel_id'=>$limitInfo['channel_id'])); //频道支持的区域
					//根据管理员权限筛选区域
					$this->admin_model = pc_base::load_model('admin_model');
					$userid = $_SESSION['userid'];
					$ad_area = $this->admin_model->get_one(array('userid'=>$userid));
