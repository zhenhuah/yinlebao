...
//设备版本信息列表
	...
	public function systemlog(){
		...
		$data  = $this->app_onlinelog->listinfo($where, $order = '`createtime` DESC', $page, $perpage);
	...
	//设置用户反馈主题
	public function feedbacktheme(){
		$this->feedback_theme = pc_base::load_model('go3capi_feedback_theme_model');
		$field    	= '*';
		$sql     	= "auth_feedback_theme WHERE 1 ";
		//$sql     	= "auth_guid_user_restrict WHERE 1 ";
		$order  	= 'ORDER BY id';
		$perpage    = isset($_GET['perpage']) && intval($_GET['perpage']) ? intval($_GET['perpage']) : 15;
		$page    	= isset($_GET['page'])    && intval($_GET['page'])    ? intval($_GET['page'])    : 1;
		$totalnum	= $this->feedback_theme->mynum($sql);
		$totalpage	= $this->feedback_theme->mytotalpage($sql, $perpage);
		$data 		= $this->feedback_theme->mylistinfo($field, $sql, $order, $page, $perpage);
		$multipage  = $this->feedback_theme->pages;
		include $this->admin_tpl('shop_feedbacktheme');
	}
	public function shop_addfeedbacktheme(){
		include $this->admin_tpl('shop_addfeedbacktheme');
	}
	public function shop_addfeedbackthemedo(){
		$this->feedback_theme = pc_base::load_model('go3capi_feedback_theme_model');
		...
		$this->feedback_theme->insert($dat);
		//添加操作日志
		$status = 'add';
		$description = $dat;
		$this->system_log($status,json_encode($description));
		$msg = '操作成功!';
		showmessage($msg,'?m=go3c&c=auth&a=feedbacktheme&pc_hash='.$_SESSION['pc_hash'], $ms = 2000);
	}
	public function shop_editfeedbacktheme(){
		$id = $_GET['id'];
		$this->feedback_theme = pc_base::load_model('go3capi_feedback_theme_model');
		$data = $this->feedback_theme->get_one(array('id'=>$id));
		include $this->admin_tpl('shop_editfeedbacktheme');
	}
	public function shop_editfeedbackthemedo(){
		$this->feedback_theme = pc_base::load_model('go3capi_feedback_theme_model');
		$id = $_POST['id'];
		$content = $_POST['content'];
		$listorder = $_POST['listorder'];
		...
		$this->feedback_theme->update($dat,array('id'=>$id));
		//添加操作日志
		$status = 'edit';
		$description = $dat;
		$this->system_log($status,json_encode($description));
	...
	public function shop_delfeedbacktheme(){
		$this->feedback_theme = pc_base::load_model('go3capi_feedback_theme_model');
		$id = $_GET['id'];
		$this->feedback_theme->delete(array('id'=>$id));
		$msg = '操作成功!';
		showmessage($msg,'?m=go3c&c=auth&a=feedbacktheme&pc_hash='.$_SESSION['pc_hash'], $ms = 2000);
	}
	//用户反馈信息
	public function feedbacklist(){
		$this->go3capi_feedback = pc_base::load_model('go3capi_feedback_model');
		$cardid = $_GET['cardid'];
		$qq = $_GET['qq'];
		$phone  = $_GET['phone'];
		$city  = $_GET['city'];
		...
		$field    	= '*';
		$sql     	= "auth_feedback ".$where;
		$order  	= ' ORDER BY id DESC';
		$perpage    = isset($_GET['perpage']) && intval($_GET['perpage']) ? intval($_GET['perpage']) : 10;
		$page    	= isset($_GET['page'])    && intval($_GET['page'])    ? intval($_GET['page'])    : 1;
		$totalnum	= $this->go3capi_feedback->mynum($sql);
		$totalpage	= $this->go3capi_feedback->mytotalpage($sql, $perpage);
		$data 		= $this->go3capi_feedback->mylistinfo($field, $sql, $order, $page, $perpage);
		$multipage  = $this->go3capi_feedback->pages;
		include $this->admin_tpl('shop_feedbacklist');
	}
	public function add_feedbackdetail(){
		$id = $_GET['id'];
		$this->go3capi_feedback = pc_base::load_model('go3capi_feedback_model');
		$data = $this->go3capi_feedback->get_one(array('id'=>$id));
		include $this->admin_tpl('shop_feedbackdetail');
	}
	//上传日志列表
	//用户反馈信息
	public function fdailylist(){
		$this->go3capi_fdaily = pc_base::load_model('go3capi_fdaily_model');
		$cardid = $_GET['cardid'];
		$mac_wire = $_GET['mac_wire'];
		$system  = $_GET['system'];
		$version  = $_GET['version'];
		...
		$field    	= '*';
		$sql     	= "auth_fdaily ".$where;
		$order  	= ' ORDER BY id DESC';
		$perpage    = isset($_GET['perpage']) && intval($_GET['perpage']) ? intval($_GET['perpage']) : 10;
		$page    	= isset($_GET['page'])    && intval($_GET['page'])    ? intval($_GET['page'])    : 1;
		$totalnum	= $this->go3capi_fdaily->mynum($sql);
		$totalpage	= $this->go3capi_fdaily->mytotalpage($sql, $perpage);
		$data 		= $this->go3capi_fdaily->mylistinfo($field, $sql, $order, $page, $perpage);
		$multipage  = $this->go3capi_fdaily->pages;
		include $this->admin_tpl('shop_fdaily');
	}
	public function fdailylistdetail(){
		$id = $_GET['id'];
		$this->go3capi_fdaily = pc_base::load_model('go3capi_feedback_model');
		$data = $this->go3capi_fdaily->get_one(array('id'=>$id));
		include $this->admin_tpl('shop_fdailyldetail');
	}
	
	//mac地址
	public function mac_wirelist(){
		$this->go3capi_mac_wire = pc_base::load_model('go3capi_mac_wire_model');
		$field    	= '*';
		$sql     	= "auth_mac_wire WHERE 1 ";
		$order  	= 'ORDER BY id';
		$perpage    = isset($_GET['perpage']) && intval($_GET['perpage']) ? intval($_GET['perpage']) : 15;
		$page    	= isset($_GET['page'])    && intval($_GET['page'])    ? intval($_GET['page'])    : 1;
		$totalnum	= $this->go3capi_mac_wire->mynum($sql);
		$totalpage	= $this->go3capi_mac_wire->mytotalpage($sql, $perpage);
		$data 		= $this->go3capi_mac_wire->mylistinfo($field, $sql, $order, $page, $perpage); 
		$multipage  = $this->go3capi_mac_wire->pages;
		include $this->admin_tpl('shop_mac_wirelist');
	}
	public function shop_delmac(){
		$this->go3capi_mac_wire = pc_base::load_model('go3capi_mac_wire_model');
		$id = $_GET['id'];
		$this->go3capi_mac_wire->delete(array('id'=>$id));
		$msg = '操作成功!';
		showmessage($msg,'?m=go3c&c=auth&a=mac_wirelist&pc_hash='.$_SESSION['pc_hash'], $ms = 2000);
	}
}