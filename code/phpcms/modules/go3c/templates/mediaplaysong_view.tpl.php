...
<script  type="text/javascript">
	var playurl='';
		<?php
				...
				if($value['vid']==$vid)
		  ?>
			playurl = "http://www.go3c.tv:8060/<?=$value['play_url']?>"
		<?php }}?>
</script>
...
<tr><td>
	...
	<tr>
      ...
      <td width="100"><?=$cms_video['name']?></td>
    </tr>
    <tr>
      <th width="80">清晰度:
	  </th>
	...
	<select name="playType" id="playType" onChange="playRouteUrl(this.value);">
		  <?php
          if(is_array($cms_video_content)) {
		  	foreach($cms_video_content as $key => $value) {
		  ?>
			<option value="http://www.go3c.tv:8060/<?php echo $value['play_url']?>" <?php if($value['vid']==$vid) echo ' selected="selected"'?>><?php echo $value['quality']?></option>
			<?php }}?>
	</select>
