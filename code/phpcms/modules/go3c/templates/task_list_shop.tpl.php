...
<div class="table-list" style="margin-top:10px;">
	...
	<?php if(!empty($task_list)){foreach($task_list as $task){?>
		...
		<td align="center"><?php echo $task['app_type'] ?></td>
		<td align="center"><?php echo $taskStatus_data[$task['taskStatus']];?></td>	
		<td align="center"><?php echo $task['spid'];?></td>
		<td align="center"><?php echo $task['board_type'];?></td>
		<td align="center" width="60">
		<a style="color:green" href="?m=go3c&c=shop&a=viewShop&mode=query&taskId=<?php echo $task['taskId'];?>">详细 </a> |
		<?php if($task['taskStatus'] == '1'||$task['taskStatus'] == '4'){?>
		<a style="color:blue" href="javascript:runTask('edit','<?php echo $task['taskId'];?>')">修改</a> | 
		<a style="color:green" href="?m=go3c&c=shop&a=showShop&taskId=<?php echo $task['taskId'];?>">添加应用 </a> |
		<a href="javascript:confirmurl('?m=go3c&c=task_shop&a=deleteTask&taskId=<?php echo $task['taskId'];?>','你确定要执行该操作吗？')">删除</a> |
		<a style="color:red" href="javascript:confirmurl('?m=go3c&c=task_shop&a=task_on&taskId=<?php echo $task['taskId'];?>','你确定要执行该操作吗？')">发布上线</a>
		<?php  } elseif ($task['taskStatus'] == '100') { ?>
		<a style="color:red" href="javascript:confirmurl('?m=go3c&c=task_shop&a=task_off&taskId=<?php echo $task['taskId'];?>','你确定要执行该操作吗？')">下线</a> |
		<a class="add fb" href="javascript:confirmurl('?m=go3c&c=task_shop&a=createtaskjson&taskId=<?php echo $task['taskId'];?>','你确定要执行该操作吗?')"><em>生成json</em></a>
		<?php } ?>
		</td>
		</tr>
	...
	//{
		switch(taskType)
		{
			case 'add':	//添加任务
				$.ajax({
					type: "GET",
					url: 'index.php?m=go3c&c=task_shop&a=addTask&pc_hash='+pc_hash,
					success: function(msg){
						art.dialog({
								content: msg,
								title:'添加任务',
			...
			case 'edit':	//修改任务
				if (taskId > 0)
				{
					$.ajax({
							type: "GET",
							url: 'index.php?m=go3c&c=task_shop&a=editTask&taskId='+taskId+'&pc_hash='+pc_hash,
							success: function(msg){
								art.dialog({
										content: msg,
										title:'修改任务',
