...
<table width="100%" cellspacing="0" class="table_form">
	<tbody>	
		...
		<tr>
		  <th width="80"> 视频ID:</th>
		  <td><?php echo $videoInfo['epgid']?> </td>
		 </tr>

		<tr>
		  <th width="80"> <font color="red">*</font> 标题:</th>
		  <td><?php echo $videoInfo['text']?></td>
		</tr>
			<tr>
		  <th width="80"> 开始时间:</th>
		  <td><?php echo $videoInfo['starttime']?>&nbsp;</td>
		</tr>


		<tr>
		  <th width="80"> 结束时间:</th>
		  <td><?php echo $videoInfo['endtime']?>&nbsp;</td>
		</tr>
		...
		<tr>
		  ...
		  <td><input type="text" class="measure-input  input-text" value="<?php echo $videoInfo['text']?>"  name="videoTitle" id="videoTitle" style="width:280px;"><span id="videoTitle_error" style="display:none;"><font color="red">此标题不能为空</font></span></td>
		</tr>
		...
		<tr>
		  <th width="120">视频播放地址类型:</th>
		  <td>
			  <select name="videoClarity" id="videoClarity">
				<option value="">请选择</option>
				<?php if(!empty($videoInfo['uuidPC_HIGH'])){?>					
					<option value="uuidPC_HIGH">清晰度uuidPC_HIGH</option>
				<?php }?>
				<?php if(!empty($videoInfo['uuidIOS_HIGH'])){?>					
					<option value="uuidIOS_HIGH">清晰度uuidIOS_HIGH</option>
				<?php }?>
				<?php if(!empty($videoInfo['uuidSTB_HIGH'])){?>					
					<option value="uuidSTB_HIGH">清晰度uuidSTB_HIGH</option>
				<?php }?>
				<?php if(!empty($videoInfo['uuidIOS_MEDUIM'])){?>					
					<option value="uuidIOS_MEDUIM">清晰度uuidIOS_MEDUIM</option>
				<?php }?>
				<?php if(!empty($videoInfo['uuidPC_MEDIUM'])){?>					
					<option value="uuidPC_MEDIUM">清晰度uuidPC_MEDIUM</option>
				<?php }?>
				<?php if(!empty($videoInfo['uuidSTB_MEDIUM'])){?>					
					<option value="uuidSTB_MEDIUM">清晰度uuidSTB_MEDIUM</option>
				<?php }?>
		...
		<tr id="isDataBase_Div"  <?php if(empty($taskInfo['imgType'])){echo 'style="display:none;"';}?>>
		  ...
		  <td>
				<?php if(!empty($channelInfo['img'])){?>					
					<input type="hidden" id="img_imgPathUrl" value="<?php echo $channelInfo['img'];?>">
				<?php }?>
				<?php if(!empty($channelInfo['imgpath'])){?>					
					<input type="hidden" id="imgpath_imgPathUrl" value="<?php echo $channelInfo['imgpath'];?>">
				<?php }?>
