...
<form name="myform" action="" method="GET">
	...
	<input type="hidden" value="task_games" name="c">
...
<div class="table-list" style="margin-top:10px;">
	...
	<?php if(!empty($task_list)){foreach($task_list as $task){?>
		...
		<a style="color:green" href="?m=go3c&c=games&a=showgame&taskId=<?php echo $task['taskId'];?>">添加游戏 </a> |
		<a style="color:green" href="?m=go3c&c=games&a=viewGame&mode=query&taskId=<?php echo $task['taskId'];?>">详细 </a> |
		<a href="javascript:confirmurl('?m=go3c&c=task_games&a=deleteTask&taskId=<?php echo $task['taskId'];?>','你确定要执行该操作吗？')">删除</a> |
		<?php if($task['taskStatus'] == '1'||$task['taskStatus'] == '3'){?>
		<a style="color:red" href="javascript:confirmurl('?m=go3c&c=task_games&a=task_on&taskId=<?php echo $task['taskId'];?>','你确定要执行该操作吗？')">发布上线</a>
		<?php  } elseif ($task['taskStatus'] == '100') { ?>
		<a style="color:red" href="javascript:confirmurl('?m=go3c&c=task_games&a=task_off&taskId=<?php echo $task['taskId'];?>','你确定要执行该操作吗？')">下线</a>
		<?php } ?>
		</td>
		</tr>
	...
	//{
		switch(taskType)
		{
			case 'add':	//添加任务
				$.ajax({
					type: "GET",
					url: 'index.php?m=go3c&c=task_games&a=addTask&pc_hash='+pc_hash,
					success: function(msg){
						art.dialog({
								content: msg,
								title:'添加任务',
			...
			case 'edit':	//修改任务
				if (taskId > 0)
				{
					$.ajax({
							type: "GET",
							url: 'index.php?m=go3c&c=task_games&a=editTask&taskId='+taskId+'&pc_hash='+pc_hash,
							success: function(msg){
								art.dialog({
										content: msg,
										title:'修改任务',
