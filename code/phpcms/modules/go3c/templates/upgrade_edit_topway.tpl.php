	...
	<table width="60%" cellspacing="0" class="table_form" style="margin-left:20%">
		<tbody>	
			...
			<tr>
				...
				<td>
					<input type="radio" name="gray" id="upgrade-all" value="0" <?php if($upgrade['is_gray']=='0') echo 'selected';?>>全部升级
					<input type="radio" name="gray" id="upgrade-gray" value="1" <?php if($upgrade['is_gray']=='1') echo 'selected';?>>灰度升级
				</td>
			</tr>
			<tr class="tr-cid-upgrade">
				...
				<td>
					<select name="cid" id="cid">
						<option value="OTT" <?php if($upgrade['cid']=='OTT') echo 'selected';?>>OTT</option>
						<option value="PVS" <?php if($upgrade['cid']=='PVS') echo 'selected';?>>PVS</option>
					</select>
				</td>
			</tr>
			...
			<tr class="tr-term-upgrade">
				...
				<td>
					<select name="gray_term_type" id="gray_term">
						<option value="66039" <?php if($upgrade['term_type']=='66039') echo 'selected';?>>66039(S812 2G 8G)</option>
						<option value="66050" <?php if($upgrade['term_type']=='66050') echo 'selected';?>>66050(S812 1G 8G)</option>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<th>升级模式</th>
				<td>
					<select name="is_mode" id="is_mode">
						<option value="1" <?php if($upgrade['is_mode']=='1') echo 'selected';?>>全量</option>
						<option value="2" <?php if($upgrade['is_mode']=='2') echo 'selected';?>>增量</option>
					</select>
			...
			<tr class="tr-gray-mode" style="display:none">
				...
				<td>		
					<select id="versionfr" name="versionfr">
						...
						<option value="<?php echo $v['versioncode']?>" <?php if($upgrade['versionfr']==$v['versioncode']) echo 'selected';?>><?php echo $v['versioncode']?></option>
						<?php }?>
					</select> to 
					<select id="versionto" name="versionto">
						<?php
						foreach ($upgradeversion as $v) {?>
						<option value="<?php echo $v['versioncode']?>" <?php if($upgrade['versionto']==$v['versioncode']) echo 'selected';?>><?php echo $v['versioncode']?></option>
						<?php }?>
					</select>
				</td>
			</tr>
...
$('#is_mode').change(function(){
	var is_mode = $.trim($('#is_mode').val());
	if(is_mode=='2'){
		$('.tr-gray-mode').show();
	}else{
		$('.tr-gray-mode').hide();
...
$(function(){
　　var tt = $('#is_mode').attr('selected',true).val();
	if(tt=='2'){
		$('.tr-gray-mode').show();
	}
});
