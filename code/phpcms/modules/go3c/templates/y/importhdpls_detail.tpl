...
<form name="myform" action="?m=go3c&c=importhdpls&a=hdpimportdetail&url={urlencode($video[url])}&t={$video[type]}" method="post" id="myform">
				...
				*}
				...
				<tr>
					<td colspan="10" style="text-align:center;">
						{if $video[count]}
						<input type="button" style="width:82px;height:25px;" href="?m=go3c&c=importhdpls&a=hdpdetaillist&url={urlencode($video[link])}&t={$video[type]}&pc_hash={$_SESSION['pc_hash']}" onclick="window.location.href=this.getAttribute('href')" value="查看分集" />
						&nbsp;&nbsp;
						{/if}
