...
<form name="myform" action="?m=go3c&c=importsso&a=importdetail&id={urlencode($video[id])}" method="post" id="myform">
	...
	<tr>
		<td style="width:50%;">
			<table width="100%" class="table_form contentWrap"><tbody>
				...
				<tr>
					<th>tags</th>
					<td><input type="text" value="{$video[tag]}" style="width:300px;" name="d[tag]" /></td>
				</tr>
				<tr>
					<th>剧情</th>
			...
			<table width="100%" class="table_form"><tbody>
				...
				<tr>
					...
					<td>
						...
						{php $v[spath]=$v['path']}
						{if $v['spath'] == 'segment'}
							{php $v['spath']="?m=go3c&c=importsso&a=showsegment&id=" .$v['id'] . '&pc_hash=' . $_SESSION['pc_hash']}
						{/if}
				...
				<tr>
					<td colspan="10" style="text-align:center;">
						{if $video[total_episodes]}
						<input type="button" style="width:82px;height:25px;" href="?m=go3c&c=importsso&a=showdetaillist&id={urlencode($video[id])}&pc_hash={$_SESSION['pc_hash']}" onclick="window.location.href=this.getAttribute('href')" value="查看分集" />
						&nbsp;&nbsp;
						{/if}
