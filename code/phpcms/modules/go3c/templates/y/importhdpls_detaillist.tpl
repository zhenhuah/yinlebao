    ...
    <tbody>
		...
		<tr>
			...
			<td align="center" class="atd">
				{if $indb_list[$v[id]]}
				<a onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this);return false;" href="?m=go3c&c=importhdpls&a=hdpimportdetaill&url={urlencode($video[url])}&t={$video[type]}&vurl[]={urlencode($v[url])}&pc_hash={$_SESSION['pc_hash']}">已导入</a>
				{else}
				<a onclick="doImportPPTVDetaill(this);return false;" href="?m=go3c&c=importhdpls&a=hdpimportdetaill&url={urlencode($video[url])}&t={$video[type]}&vurl[]={urlencode($v[url])}&pc_hash={$_SESSION['pc_hash']}">导入</a>
				{/if}
			</td>
		</tr>
		{/loop}
...
function doShowOk(){
	...
	window.location.href = '?m=go3c&c=importhdpls&a=showOK&pc_hash={$_SESSION['pc_hash']}' ;
}
