    ...
    <tbody>
		...
		<tr>
			...
			<td align="center" class="atd">
				...
				{else}
				<a onclick="doImportPPTVDetaill(this);return false;" href="?m=go3c&c=importhdpyk&a=hdpimportdetaill&url={urlencode($video[url])}&t={$video[type]}&vurl[]={urlencode($v[url])}&pc_hash={$_SESSION['pc_hash']}">导入</a>
				{/if}
			</td>
		</tr>
		{/loop}
...
function doShowOk(){
	...
	window.location.href = '?m=go3c&c=importhdpyk&a=showOK&pc_hash={$_SESSION['pc_hash']}' ;
}
