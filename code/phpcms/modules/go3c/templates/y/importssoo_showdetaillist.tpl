   ...
   <thead><tr>
    ...
    <tbody>
		...
		<tr>
			...
			<td align="center">
				{if $indb_list[$v[VID]]}
				<a onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this);return false;" href="?m=go3c&c=importssoo&a=importdetaill&id={urlencode($video[id])}&vid[]={urlencode($v[VID])}&pc_hash={$_SESSION['pc_hash']}">已导入</a>
				{else}
				<a onclick="doImportPPTVDetaill(this);return false;" href="?m=go3c&c=importssoo&a=importdetaill&id={urlencode($video[id])}&vid[]={urlencode($v[VID])}&pc_hash={$_SESSION['pc_hash']}">导入</a>
				{/if}
			</td>
		</tr>
		{/loop}
...
function doShowOk(){
	...
	window.location.href = '?m=go3c&c=importssoo&a=showOK&pc_hash={$_SESSION['pc_hash']}' ;
}
