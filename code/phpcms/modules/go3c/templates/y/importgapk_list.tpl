...
<form name="myfrom" action="" method="GET">
	...
	<input type="hidden" value="importgapk" name="c">
	<input type="hidden" value="apklist" name="a">
   ...
   <thead><tr>
    ...
    <tbody>
		...
		<tr>
			...
			<td align="center">
				<a href="?m=go3c&c=importgapk&a=hdpdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="popShowDetail(this.href);return false;">详情</a>
			</td>
			<td align="center">
			{if $indb_list[$v['sid']]}
			<a href="?m=go3c&c=importgapk&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this,'?m=go3c&c=importgapk&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">已导入</a>
			<a href="?m=go3c&c=importgapk&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}&pc_hash={$_SESSION[pc_hash]}" onclick="doImportHDPDetaill(this);return false;">更新</a>	
			{else}
			<a href="?m=go3c&c=importgapk&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="doImportPPTVDetaill(this,'?m=go3c&c=importgapk&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">导入</a>
			{/if}		
			</td>
		</tr>
		{/loop}
