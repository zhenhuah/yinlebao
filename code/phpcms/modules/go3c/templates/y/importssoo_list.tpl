...
<div class="content-menu ib-a blue line-x">
	<a class="add fb" href="javascript:doimportssoo();" ><em>导入网信源数据..</em></a>
	<span id="a_doimportssoo" style="color:red"></span>
</div>
-->
<div class="explain-col">
<form name="myfrom" action="" method="GET">
	<input type="hidden" value="go3c" name="m">
	<input type="hidden" value="importssoo" name="c">
...
类型:
  ...
  <select name="w[vcolumn]" id="i_w_vcolumn" onchange="doChangeType(this.value);">
...
视频资源类别：
{php $videotypes=array('flv'=>'flv','m3u8'=>'m3u8','mp4'=>'mp4','segment'=>'多段','hdpfans'=>'跳转','other'=>'其他','none'=>'无')}
<select name="w[vt]" id="i_vt">
<option value="">不限</option>
{loop $videotypes $k $v}
<option value="{$k}"{if !empty($w[vt]) && $w[vt] == $k} selected="selected"{/if}>{$v}</option>
{/loop}
</select>  &nbsp;
  ...
  {/if}
	...
	{if !empty($aconf[sourcenames])}
	来源网站：<select name="w[sname]" id="i_w_sname">
			<option value="">不限</option>
			{loop $aconf[sourcenames] $v}
			{php if(empty($v)) $v='未知'}
			{php $k=$v}
			
			<option value="{$k}"{if !empty($w[sname]) && $w[sname] == $k} selected="selected"{/if}>{$v}</option>
			{/loop}
		</select>  &nbsp; 
...
<script>
function doimportssoo(){
	$('#a_doimportssoo').html('正在导入') ;
	location.href = '?m=go3c&c=importssoo&a=doimportssoo&pc_hash='+pc_hash+'&goback='+BASE64.encode(location.search);
}
function i$(i){ return document.getElementById(i); }
function doChangeType(vv){
	var dds = i$('i_w_cate_in').getElementsByTagName('span') ;
	for(var i=0;i<dds.length;i++){
		i$('i_w_cate_in_display_none').appendChild(dds[i]) ;
	}

	var t = i$('i_w_cate_'+vv+'_span')
	if(t) i$('i_w_cate_in').appendChild(t) ;
}
doChangeType('{$w[vcolumn]}') ;
...
<table width="100%" cellspacing="0" id="i_table_list_1">
   <thead><tr>
		...
		<th align="center" style="width:100px;">分类</th>
		{if !empty($aconf[area])}
		...
		<th style="width:130px;" align="center">操作</th>
    </tr></thead>
    <tbody>
		{loop $list $v}
		...
		<tr id="i_import_tr_{$v[id]}">
			...
			{/if}
			<td align="center">
				<a href="?m=go3c&c=importssoo&a=showdetail&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" onclick="popShowDetail(this.href);return false;">详情</a>
				&nbsp;&nbsp;
				{if $v[vcolumn] != '电影'}
				<a href="?m=go3c&c=importssoo&a=showdetaillist&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" onclick="popShowDetail(this.href);return false;">查看分集</a>
				&nbsp;&nbsp;
				<a href="?m=go3c&c=importssoo&a=importdetaill&id={urlencode($v[id])}&vid=all&skipin=1&pc_hash={$_SESSION[pc_hash]}" onclick="doImportPPTVDetaill(this);return false;">导入新增分集</a>
				&nbsp;&nbsp;
				{/if}
			...
			<td align="center">
			{if $indb_list[$v['VID']]}
			<a href="?m=go3c&c=importssoo&a=importdetail&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_import_{$v[id]}" onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this);return false;">已导入</a>
			{else}
			<a href="?m=go3c&c=importssoo&a=importdetail&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_import_{$v[id]}" onclick="doImportPPTVDetaill(this);return false;">导入</a>
			{/if}
			&nbsp;
			<a href="?m=go3c&c=importssoo&a=dels&ids[]={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_del_{$v[id]}" onclick="if(confirm('确认？')) doImportPPTVDetaill(this);return false;">删除</a>
			

			&nbsp;
			<a href="?m=go3c&c=importssoo&a=upAV&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_upv_{$v[id]}" at="upva" class="aaupv" onclick="doImportPPTVDetaill(this);return false;">更新</a>
