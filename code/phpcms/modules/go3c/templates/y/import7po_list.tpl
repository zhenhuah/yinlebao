...
<form name="myfrom" action="" method="GET">
	...
	<input type="hidden" value="import7po" name="c">
	<input type="hidden" value="polist" name="a">
	

  类型：<select name="w[type]" id="i_w_type" onchange="doChangeType(this.value);">
				<option value="">请选择</option>
				{loop $base_data[cate][0][l] $k $v}
...
<table width="100%" cellspacing="0">
   <thead><tr>
		<th style="width:100px;" align="center">VID</th>
		<th style="width:100px;" align="center">片名</th>
		<th align="center" style="width:60px;">图片</th>
    ...
    <tbody>
		...
		<tr>
			...
			<td align="center">
				<a href="?m=go3c&c=import7po&a=hdpdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="popShowDetail(this.href);return false;">详情</a>
			</td>
			<td align="center">
			{if $indb_list[$v['app_name']]}
			<a href="?m=go3c&c=import7po&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this,'?m=go3c&c=import7po&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">已导入</a>
			<a href="?m=go3c&c=import7po&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}&pc_hash={$_SESSION[pc_hash]}" onclick="doImportHDPDetaill(this);return false;">更新</a>	
			{else}
			<a href="?m=go3c&c=import7po&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="doImportPPTVDetaill(this,'?m=go3c&c=import7po&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">导入</a>
			{/if}		
			</td>
		</tr>
		{/loop}
...
function popShowDetail(url){
	window.top.art.dialog({title:'apk资源详情', id:'edit', iframe:url,width:'800px',height:'450px'},'', function(){window.top.art.dialog({id:'edit'}).close()});
}
