...
<form name="myfrom" action="" method="GET">
	...
	<input type="hidden" value="importsso" name="c">
   ...
   <thead><tr>
    ...
    <tbody>
		...
		<tr id="i_import_tr_{$v[id]}">
			...
			{/if}
			<td align="center">
				<a href="?m=go3c&c=importsso&a=showdetail&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" onclick="popShowDetail(this.href);return false;">详情</a>
				&nbsp;&nbsp;
				{if $v[vcolumn] != '电影'}
				<a href="?m=go3c&c=importsso&a=showdetaillist&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" onclick="popShowDetail(this.href);return false;">查看分集</a>
				&nbsp;&nbsp;
				<a href="?m=go3c&c=importsso&a=importdetaill&id={urlencode($v[id])}&vid=all&skipin=1&pc_hash={$_SESSION[pc_hash]}" onclick="doImportPPTVDetaill(this);return false;">导入新增分集</a>
				&nbsp;&nbsp;
				{/if}
			...
			<td align="center">
			{if $indb_list[$v['VID']]}
			<a href="?m=go3c&c=importsso&a=importdetail&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_import_{$v[id]}" onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this);return false;">已导入</a>
			{else}
			<a href="?m=go3c&c=importsso&a=importdetail&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_import_{$v[id]}" onclick="doImportPPTVDetaill(this);return false;">导入</a>
			{/if}
			&nbsp;
			<a href="?m=go3c&c=importsso&a=dels&ids[]={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_del_{$v[id]}" onclick="if(confirm('确认？')) doImportPPTVDetaill(this);return false;">删除</a>
			
			{if $v[vcolumn] == '电影'}
			&nbsp;
			<a href="?m=go3c&c=importsso&a=upAVRSS&ids[]={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_upv_{$v[id]}" at="upva" class="aaupv" onclick="doImportPPTVDetaill(this);return false;">更新</a>
			{else}
			&nbsp;
			<a href="?m=go3c&c=importsso&a=showdetaillist&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" hrefinit="?m=go3c&c=importsso&a=upAVC&id={urlencode($v[id])}&pc_hash={$_SESSION[pc_hash]}" id="a_upv_{$v[id]}" at="upvs" class="aaupv" onclick="doUPVDetaillList(this);return false;">更新</a>
			{/if}
			</td>
		</tr>
		{/loop}
