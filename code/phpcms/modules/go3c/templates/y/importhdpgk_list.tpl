...
<form name="myfrom" action="" method="GET">
	...
	<input type="hidden" value="importhdpgk" name="c">
  ...
  类型：<select name="w[type]" id="i_w_type" onchange="doChangeType(this.value);">
				...
				<option value="{$base_data[type][name]}"{if $w[type] == $base_data[type][name]} selected="selected"{/if}>{$base_data[type][name]}</option>
		</select>  &nbsp;  
   ...
   <thead><tr>
    ...
    <tbody>
		...
		<tr>
			...
			<td align="center">
				<a href="?m=go3c&c=importhdpgk&a=hdpdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="popShowDetail(this.href);return false;">详情</a>
				&nbsp;&nbsp;
				{if $v[type] != 'movie'}
				<a href="?m=go3c&c=importhdpgk&a=hdpdetaillist&url={urlencode($v[url])}&t={$w[type]}" onclick="popShowDetail(this.href);return false;">查看分集</a>
				&nbsp;&nbsp;
				<a href="?m=go3c&c=importhdpgk&a=hdpimportdetaill&url={urlencode($v[url])}&vid=all&t={$w[type]}" onclick="doImportPPTVDetaill(this);return false;">导入新增分集</a>
				&nbsp;&nbsp;
				{/if}
			...
			<td align="center">
			{if $indb_list[$v['id']]}
			<a href="?m=go3c&c=importhdpgk&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this,'?m=go3c&c=importhdpgk&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">已导入</a>
			<a href="?m=go3c&c=importhdpgk&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}&pc_hash={$_SESSION[pc_hash]}" onclick="doImportHDPDetaill(this);return false;">更新</a>	
			{else}
			<a href="?m=go3c&c=importhdpgk&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="doImportPPTVDetaill(this,'?m=go3c&c=importhdpgk&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">导入</a>
			{/if}		
			</td>
		</tr>
		{/loop}
