...
<form name="myfrom" action="" method="GET">
	...
	<input type="hidden" value="importhdpyy" name="c">
...
<table width="100%" cellspacing="0">
   <thead><tr>
		...
		<th style="width:150px;" align="center">歌名</th>
    ...
    <tbody>
		...
		<tr>
			...
			<td align="center">
				<a href="?m=go3c&c=importhdpyy&a=hdpdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="popShowDetail(this.href);return false;">详情</a>
			</td>
			<td align="center">
			{if $indb_list[$v['id']]}
			<a href="?m=go3c&c=importhdpyy&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="if(confirm('确定重新导入？')) doImportPPTVDetaill(this,'?m=go3c&c=importhdpyy&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">已导入</a>
			<a href="?m=go3c&c=importhdpyy&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}&pc_hash={$_SESSION[pc_hash]}" onclick="doImportHDPDetaill(this);return false;">更新</a>	
			{else}
			<a href="?m=go3c&c=importhdpyy&a=hdpimportdetail&url={urlencode($v[url])}&t={$w[type]}" onclick="doImportPPTVDetaill(this,'?m=go3c&c=importhdpyy&a=hdpdetaillist&url={urlencode($v[url])}&vid=all&t={$w[type]}&pc_hash={$_SESSION['pc_hash']}');return false;">导入</a>
			{/if}		
			</td>
		</tr>
		{/loop}
