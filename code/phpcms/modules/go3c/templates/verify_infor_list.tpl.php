...
<input type="hidden" value="infor_online" name="a">
...
<table width="100%" cellspacing="0" class="search-form">
    <tbody>
		...
		<div class="explain-col">
		    <a href="?m=go3c&c=verify&a=infor_online">全部数据</a>&nbsp;
...
<div class="table-list">
	...
	<td align="center"><?php echo $client_version['title'];?></td>
	 <td align="center"><?php {foreach($type_name_list as $key=>$value){?>
	 <?php if($value['id']==$client_version['type']) echo $value['type_name'];?>
	<?php }} ?></td>
...
function dopass(id) {
    location.href ='?m=go3c&c=infor&a=infor_passdo&id='+id+'&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}
function dorefuse(id) {
    location.href ='?m=go3c&c=infor&a=infor_passdo_refuse&id='+id+'&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}
</script>
