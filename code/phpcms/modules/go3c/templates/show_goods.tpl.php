...
<input type="hidden" value="show_goods" name="a">
...
<table width="100%" cellspacing="0" class="search-form">
	...
	<tr>
		...
		<div class="explain-col">
			...
			<?php if(is_array($goods_data) ){foreach($goods_data as $v){?> 
				<option value="<?php echo $v['name'] ?>" <?php if($type == $v['name']){ echo 'selected';}?> >
					<?php echo $v['name'] ?>
				</option> 	
			<?php }} ?>	
			</select>
			商品名称：<input id= "name" name="name" type="text" value="<?php if(isset($name)) echo $name;?>" class="input-text" size="15"/>&nbsp;
...
<div class="table-list">
    <table width="100%" cellspacing="0" id="i_table_list_1">
        <thead>
            <tr>
				...
				<th width='20' align="left">商品类别</th>
	...
	<tr>
		...
		<td align="left"><?php echo $v['sign']?></td>
...
function doCKALL(){
    ...
    if(str!=''){
    	...
    	if(doitem == '2'){  
			...
			location.href ='?m=go3c&c=tvuser&a=delete_goods&id='+str+'&pc_hash='+pc_hash
...
function addnew(){
	location.href ='?m=go3c&c=tvuser&a=addGoods&catid=&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}
function dodelete(id) {
    location.href ='?m=go3c&c=tvuser&a=delete_goods&id='+id+'&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}
function edit(id){
	location.href ='?m=go3c&c=tvuser&a=goods_edit&id='+id+'&pc_hash='+pc_hash+'&goback='+BASE64.encode(location.search);
}
</script>
