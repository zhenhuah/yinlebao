...
<div style="text-align:right;padding:0 50px 10px 0;">
  <input class="button" type="button" onclick="dopassall('确认批量通过删除申请？')" value="批量通过删除申请" />&nbsp;&nbsp;
  <input class="button" type="button" onclick="dorefuseall('确认批量拒绝删除申请？')" value="批量拒绝删除申请" />
</div>
-->
...
<table width="100%" cellspacing="0" class="search-form">
    <tbody>
		...
		<div class="explain-col">
		    <a href="?m=go3c&c=verify&a=delete">全部数据</a>&nbsp;
...
<div class="table-list">
	...
	<td align="center">
		<a style="color:green" href="javascript:dopass(<?php echo $channel['id']?>, '通过删除')">通过</a>
		&nbsp;&nbsp;
		<a style="color:red" href="javascript:dorefuse(<?php echo $channel['id']?>, '驳回删除')">拒绝</a>
...
function doCKALL(){
    ...
    if(str!=''){
		if(doitem == '2'){  //拒绝
			location.href ='?m=go3c&c=verify&a=delete_refuse_all&id='+str+'&pc_hash='+pc_hash
	    	+'&goback='+BASE64.encode(location.search);
		}else if(doitem == '1'){  //同意
			location.href ='?m=go3c&c=verify&a=delete_pass_all&id='+str+'&pc_hash='+pc_hash
	    	+'&goback='+BASE64.encode(location.search);
		}
    }else{
    	alert('你还没有选择任何内容！');
...
function dopass(id, title) {
    location.href ='?m=go3c&c=verify&a=delete_pass&id='+id+'&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}
function dorefuse(id, title) {
    location.href ='?m=go3c&c=verify&a=delete_refuse&id='+id+'&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}

function dopassall(title){
	if(confirm(title)){		
		document.myform.action="?m=go3c&c=verify&a=delete_pass_all&goback="+BASE64.encode(location.search);
		document.myform.submit(); 
	}
}
function dorefuseall(title){
	if(confirm(title)){		
		document.myform.action="?m=go3c&c=verify&a=delete_refuse_all&goback="+BASE64.encode(location.search);
		document.myform.submit(); 
	}
}
</script>
