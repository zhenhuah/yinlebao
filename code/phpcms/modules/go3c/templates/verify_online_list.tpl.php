...
<div style="text-align:right;padding:0 50px 10px 0;">
  <input class="button" type="button" onclick="dopassall('确认批量通过上线申请？')" value="批量通过上线申请" />&nbsp;&nbsp;
  <input class="button" type="button" onclick="dorefuseall('确认批量拒绝上线申请？')" value="批量拒绝上线申请" />
</div>
 -->
...
function doCKALL(){
    ...
    if(str!=''){
		if(doitem == '2'){  //拒绝
			location.href ='?m=go3c&c=verify&a=delete_allto&id='+str+'&pc_hash='+pc_hash
	    	+'&goback='+BASE64.encode(location.search);
		}else if(doitem == '1'){  //上线
			location.href ='?m=go3c&c=verify&a=online_passto_all&id='+str+'&pc_hash='+pc_hash
	    	+'&goback='+BASE64.encode(location.search);
		}
    }else{
    	alert('你还没有选择任何内容！');
...
function dopass(id, title) {
    location.href ='?m=go3c&c=verify&a=online_pass&id='+id+'&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}
function dorefuse(id, title) {
    location.href ='?m=go3c&c=verify&a=online_refuse&id='+id+'&pc_hash='+pc_hash
	+'&goback='+BASE64.encode(location.search);
}
function dopassall(title){
	if(confirm(title)){		
		document.myform.action="?m=go3c&c=verify&a=online_pass_all&goback="+BASE64.encode(location.search);
		document.myform.submit(); 
	}
}
function dorefuseall(title){
	if(confirm(title)){		
		document.myform.action="?m=go3c&c=verify&a=online_refuse_all&goback="+BASE64.encode(location.search);
		document.myform.submit(); 
	}
}
</script>
