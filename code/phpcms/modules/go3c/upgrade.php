...
class upgrade extends admin {
	...
	public function upgrade_add_topwaydo(){
		...
		if($is_mode=='2'){
			...
			$where = " 1 order by versioncode desc limit 1";
			$data11  = $this->client_upgrade->select($where);
			$versionto = $data11[0]['versioncode'];
			$data = array(
				'upgrade_type' => $upgrade_type,
				'u_status' => $status,	//2:新添加 待审核状态 0:pad / phone
				'is_gray' => $gray,
	...
	public function veragree_upgrade(){
		...
		if($data['is_mode']=='1'){   //全量(把以前的全部置为历史)
			$upgrade_type = 'FIRMWARE';
			$this->client_upgrade->update(array('u_status'=>0), array('upgrade_type'=>$upgrade_type, 'cid'=>'topway', 'u_status'=>1, 'is_mode'=>1));
