...
class import extends admin {
	...
	public function imdodo() {
		...
		if($do_epg){
			if (empty($p)) {
				$systemdate = date('Y-m-d');  //取当天的日期
			}else{
				$systemdate=$p;
			}
			$dtime = strtotime($systemdate)-86400*7;
			$ddate = date('Y-m-d',"$dtime");
			$stime = strtotime($systemdate)-86400*2;
			$sdate = date('Y-m-d',"$stime");
			$db->d('go3c_program', array('systemdate'=>$sdate));
			$phpcmsdb->d('v9_channelepg', array('systemdate'=>$ddate));
		}
	...
	public function doscript(){
		...
		if($imchannel){
			...
			if($d['result']=='no'){
				showmessage('赛维安讯导入频道接口异常!',$HTTP_REFERER, $ms = 600);
			}
			$d['t'] = $imchannel ;
		...
		if($imasset){
			...
			if($d['result']=='no'){
				showmessage('赛维安讯导入视频接口异常!',$HTTP_REFERER, $ms = 600);
			}
			$d['t'] = $imasset ;
