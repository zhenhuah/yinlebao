...
class position_info extends admin {
	...
	public function init() {
			...
			$where .= " AND type_id > 1100 AND type_id < 2000";
			//echo $where;
			...
			$pages = $this->db->pages;
			$show_dialog = true;
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=go3c&c=position_info&a=add\', title:\'添加推荐位\', width:\'500\', height:\'360\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加推荐位');
			include $this->admin_tpl('position_list_info');
	}
	...
	public function add() {
		...
		$term_type_list = $this->term_type->select($where = '', $data = 'id,title', $limit = '', $order = 'id ASC', $group = '', $key='');
		$postion_type_list = $this->postion_type->select($where = '', $data = 'id,title', $limit = '', $order = 'id ASC', $group = '', $key='');
		...
		foreach($postion_type_list as $pvalue){
			$postion_type_array[$pvalue['id']]=$pvalue['title'];
		}
		...
		if(isset($_POST['dosubmit'])) {
			...
			include $this->admin_tpl('position_add_info');
		}	
	}
	...
	public function delete() {
		...
		$this->_set_cache();
		showmessage(L('posid_del_success'),'?m=go3c&c=position_info');
	}
