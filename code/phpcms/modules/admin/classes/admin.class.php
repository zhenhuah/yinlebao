...
class admin {
	 ...
	 * 记录工作人员cms操作日志
	 */
	/*final public function go3ccms_changlog($id,$type,$contetdec=''){
		$this->cmslog = pc_base::load_model('cms_cmslog_model');
		$userid = $_SESSION['userid'];
		$this->spid_db = pc_base::load_model('admin_model');
		$user = $this->spid_db->get_one(array('userid'=>$userid));
		$ip = ip();
		$datalog = array(
				'vid' =>$id,
				'contetdec' =>$contetdec,
		...
		$this->cmslog->insert($datalog, true);
	}*/
	/*
	*	记录后台系统操作日志
	*/
	final public function system_log($status,$description){
		...
		$this->app_onlinelog->insert($datalog, true);
	}
