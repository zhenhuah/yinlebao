...
class position extends admin {
	...
	public function add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(empty($_POST['infoa']['area_idq'])){
				if(empty($_POST['infoa']['area_idc'])){
					if(empty($_POST['infoa']['area_ids'])){
						$area_id = '01';
					}else{
						$area_id = $_POST['infoa']['area_ids'];
					}
				}else{
					$area_id = $_POST['infoa']['area_idc'];
				}
			}else{
				$area_id = $_POST['infoa']['area_idq'];
			}
			$_POST['info']['area_id'] = $area_id;
			...
			$area_list = $this->cms_area->select("m = 1", 'id,name', '', 'id ASC');
	...
	public function selarea(){
		...
		$limitInfo = $this->cms_area->select($where, $data = 'id,name', $limit = '', $order = 'id ASC', $group = '', $key='');
