...
class role extends admin {
	...
	public function init() {
		$infos = $this->db->select($where = '', $data = '*', $limit = '', $order = 'listorder DESC, roleid ASC', $group = '');
	...
	public function role_priv() {
		...
		if(isset($_POST['dosubmit'])){
			if (is_array($_POST['menuid']) && count($_POST['menuid']) > 0) {
				...
				//加上详细的操作权限(添加/修改/删除/审核--针对多项目账号)
				$pre = array('boot'=>'advertStatus','boot'=>'editAdvert','boot'=>'addAdvert','boot'=>'addremote',
					'boot'=>'addremotedo','boot'=>'editremote','boot'=>'editremotedo','boot'=>'delremote',
					'boot'=>'createremote','task'=>'addTask','task'=>'editTask','task'=>'copytask',
					'task'=>'addOutTask','task'=>'showVideo','task'=>'video',
					'task'=>'deleteTask','task'=>'addVideo','task'=>'verifyVideo','task'=>'editVideo',
					'task'=>'deleteVideo','task'=>'listorder','task'=>'addAdvert','task'=>'editAdvert',
					'task'=>'deleteAdvert','task'=>'verifyAdvert','client'=>'versionlist','client'=>'preview');
				foreach ($pre as $key=>$v){
					$role = array(
						'roleid'=> $_POST['roleid'],
						'm'		=> 'go3c',
						'c'		=> $key,
			...
			if ($siteid) {
				...
				$result =$this->menu_db->select(array('display'=>1),'*',1000,'listorder ASC');
				//var_dump($result);
	...
	public function spidlist(){
		...
		$where = " 1 ";
		if($spid) $where = "spid = '$spid'";
		if($custname) $where = "custname = '$custname'";
		$page  = $_GET['page'] ? $_GET['page'] : '1';
		$perpage = intval($_GET['perpage']) ? intval($_GET['perpage']) : '15';
		$spiddata  = $this->spid_db->listinfo($where, $order = "$field $order", $page, $perpage);
		$pages = $this->spid_db->pages;
		include $this->admin_tpl('spidlist');
	}
	//添加项目模型
	public function add_spid(){
		include $this->admin_tpl('spid_add');
	}
	//添加项目操作
	public function adddo_spid(){
		$this->spid_db = pc_base::load_model('cms_spid_model');
		$spid  = $_POST['spid'];
		...
		$this->spid_db->insert($sdata);
		showmessage(L('operation_success'));
	}
	//编辑项目模型
	public function edit_spid(){
		$id  = $_GET['id'];
		$this->spid_db = pc_base::load_model('cms_spid_model');
		$spkey = "id = '$id'";
		$data = $this->spid_db->get_one($spkey);
		include $this->admin_tpl('spid_edit');
	}
	//编辑项目操作
	public function editdo_spid(){
		$this->spid_db = pc_base::load_model('cms_spid_model');
		$id  = $_POST['id'];
		$spid  = $_POST['spid'];
		$custname  = $_POST['custname'];
		...
		$this->spid_db->update($sdata,$swhere);
		showmessage(L('operation_success'));
	}
	//删除项目
	public function deletespid(){
		$id  = $_GET['id'];
		$this->spid_db = pc_base::load_model('cms_spid_model');
		$swhere = array(
				'id'=>$id,
		);
		$this->spid_db->delete($swhere);
		showmessage(L('operation_success'));
	}
}
?>