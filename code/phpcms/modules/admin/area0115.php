...
class area extends admin {
	...
	public function addareado(){
		...
		$this->cms_area = pc_base::load_model('cms_area_model');
		$this->tv_area_model = pc_base::load_model('tv_area_model');
		$iKey = "id = '$id'";
		...
		$this->tv_channel_area_mapping_model->delete(array('area_id'=>$id));
		$cms_channel = $this->cms_channel->select();
		foreach ($cms_channel as $vc){
			$cd =array(
				'channel_id' => $vc['channel_id'],
				'area_id' => $id,
				'category_id' => $vc['channel_category'],
				'seq_number' => '0',
				'isextend' => '0'
			);
			$this->tv_channel_area_mapping_model->insert($cd);
		}
		$this->cms_area->insert($data);
		$this->tv_area_model->insert($gda);
		//添加操作日志
		$type = 'add_area';
		$this->go3ccms_changlog($id,$type,json_encode($data));
	...
	public function editareado(){
		...
		$this->tv_area_model->update($gda,array('area_id'=>$id));
		//添加操作日志
		$type = 'edit_area';
		$this->go3ccms_changlog($id,$type,json_encode($data));
		showmessage('操作成功','index.php?m=admin&c=area&a=index');	
	...
	public function delete_area(){
		...
		$this->tv_area_model->delete(array('area_id'=>$id));
		//添加操作日志
		$type = 'del_area';
