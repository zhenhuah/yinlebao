...
echo '<ul class="page-sidebar-menu page-sidebar-menu-closed" data-keep-expanded="false" data-auto-scroll="true" data-slide-speed="200">
...
foreach($datas as $_value) {
	...
	foreach($sub_array as $_key=>$_m) {
		...
		if($_m['type']==0){
			$hyperlinks = $_m['hyperlinks'];
			$v = 'http';
			if(strpos($hyperlinks,$v) === false){
				echo '<li id="_MP'.$_m['id'].'" '.$classname.'><a href="'.menu_path.$_m['hyperlinks'].'" hidefocus="true" style="outline:none;" target="right"><i class="'.$icon_arr[$index].'"></i>'.L($_m['name']).'</a></li>';
			}else{
				echo '<li id="_MP'.$_m['id'].'" '.$classname.'><a href="'.$_m['hyperlinks'].'" hidefocus="true" style="outline:none;" target="right"><i class="'.$icon_arr[$index].'"></i>'.L($_m['name']).'</a></li>';
			}
		}else{
