...
class index extends admin {
	public function __construct() {
		...
		$this->admin_role = pc_base::load_model('admin_role_model');
		$this->cms_message = pc_base::load_model('cms_message_model');
	...
	public function init () {
		...
		//获取系统的消息
		$noti = array();$message = array();$task = array();
		//$allmess = $this->cms_message->select(array('userid'=>$userid), "*",'', 'm_time');
		$allmess = $this->cms_message->select($where = 'userid = '.$userid.' and status=1', "*",'', $order = 'm_time desc');
		foreach($allmess as $v){
			if($v['type']=='notifications'){
				$noti[]=$v;
			}elseif($v['type']=='Messages'){
				//系统消息查询来源者
				$source = $this->db->get_one(array('username'=>$v['fromuser']));
				$v['imageurl'] = $source['imageurl'];
				$message[]=$v;
			}elseif($v['type']=='task'){
				$task[]=$v;
			}
		}
		$nnum = count($noti);$mnum = count($message);$tnum = count($task);
		$this->db->update(array('tnum'=>$tnum),array('userid'=>$userid));
		/*管理员收藏栏*/
	...
	public function login() {
		if(isset($_GET['dosubmit'])) {
			...
			$admin_status = $this->db->get_one(array('username'=>$username));
			if ($admin_status['status'] == '0') {
				showmessage('用户已被锁定 请联系管理员解锁',HTTP_REFERER);
			}
			...
			$role = $this->admin_role->get_one(array('roleid'=>$r['roleid']));
			...
			$_SESSION['spid'] = $r['spid'];
			$_SESSION['username'] = $r['username'];
			$_SESSION['upgrade_role'] = $role['upgrade_role'];
			$_SESSION['imageurl'] = $r['imageurl'];
	...
	public function public_main() {
		...
		foreach($allmess as $v){
			if($v['type']=='notifications'){
				...
				$source = $this->db->get_one(array('username'=>$v['fromuser']));
				$v['fusername'] = $source['username'];
				$task[]=$v;
			}elseif($v['type']=='support'){
				$source = $this->db->get_one(array('username'=>$v['fromuser']));
				$v['fusername'] = $source['username'];
				$v['imageurl'] = $source['imageurl'];
				$support[]=$v;
		...
		//查询会员相关信息
		$member = $this->member_model->select();
		$tnum = count($task);$snum = count($support);
