...
function __bigtvm_import_doiscript_imchannel_funcr($r){
	...
	$d = array(
		...
		'uuidIOS_HIGH'     => get_live_url('IOS_HIGH',$r['uuidSTB_MEDIUM']),
		'uuidSTB_HIGH' 	   => get_live_url('STB_HIGH',$r['uuidSTB_HIGH']),
		'uuidPC_MEDIUM'    => get_live_url('PC_MEDIUM',$r['uuidPC_MEDIUM']),
		'uuidIOS_MEDUIM'   => get_live_url('IOS_MEDUIM',$r['uuidSTB_MEDIUM']),
...
function __bigtvm_import_doiscript_imepg_funcr($r){
	...
	$d = array(
		...
		//'uuidIOS_HIGH'     => get_timeshift_url('IOS',$r['uuidSTB_MEDIUM']),
		//'uuidSTB_HIGH' 	   => get_timeshift_url('STB',$r['uuidSTB_HIGH']),
		//'uuidPC_MEDIUM'    => get_timeshift_url('PC',$r['uuidPC_MEDIUM']),
		//'uuidIOS_MEDUIM'   => get_timeshift_url('IOS',$r['uuidSTB_MEDIUM']),
...
function __bigtvm_import_doscript_imasset($imasset,$s=false){
	...
	//luolei add: 20140508 soooner add param to set start date and end date 
	$stime = time()-86400*7;
	if(USE_ONLINE_API) $iurl = 'http://111.208.56.196:8028/bsmproo/export2portal/export?spid='.$spid.'&begintime='.date('Ymd',"$stime").'&endtime='.date('Ymd',time());
