...
function __do_publish_to_video($r){
	...
	if( $r['pub_date'] == '0000-00-00' || $r['pub_date_time'] <= time()){
		...
		if($r['episode_number']){
			...
			if($r['column_id']=='3'){
				$episode_number = $r['episode_number'];
				$len = strlen($episode_number);
				if($len==8){
					$ye = substr($episode_number,0,4).'-'.substr($episode_number,4,2).'-'.substr($episode_number,6,2).' 00:00:00';
					$d['time_updated']= $ye;
				}else{
					$d['time_updated']= date('Y-m-d H:i:s',time());
				}
			}
		}
		...
		if($r['parent_id']){
			...
			if($r['episode_number']&&$r['column_id']=='3'){
				$episode_number = $r['episode_number'];
				$len = strlen($episode_number);
				if($len==8){
					$ye = substr($episode_number,0,4).'-'.substr($episode_number,4,2).'-'.substr($episode_number,6,2).' 00:00:00';
					...
					$tt['time_updated']= date('Y-m-d H:i:s',time());
				}
			}
		...
		foreach ($im as $iv){
			...
			if(empty($r['parent_id'])&&(($r['ispackage']=='1')||($r['ispackage']=='0'))){
				//增加主信息到零时表(总集)
				$vd = array(
						'vid' => $r['asset_id'],
						'name' => $r['title'],
			...
			foreach ($im as $ims){
				...
				if($r['episode_number']&&$r['column_id']=='3'){
					...
					if($len==8){
						...
						$dv['time_updated']= date('Y-m-d H:i:s',time());
					}
				}
...
function __do_publish_to_pre_adverts($r){
	...
	*/
	...
	$log = array(
		...
		'version'   => $r['version'],
		'url'   => $r['imgUrl'],
		'ad_belong'   => $r['ad_belong'],
		'term'   => $r['term_id'],
		'fid'   => '2',
		'createtime'   => time()
	);
	$phpcmsdb->w('v9_cmslog',$log);
...
function __do_publish_to_information($r){
	...
	$sumcount++ ;
	if(!empty($r['id'])){
		$inarea = $phpcmsdb->r('v9_information_area',array('nid'=>$r['id'])) ;
		foreach ($inarea as $va){
			$dar = array(
					'nid' 		=> $r['id'],
					'title' 	=> $r['title'],
					'type' 		=> $r['type'],
			...
			$go3cdb->w('information',$dar);
		}
	}
	//更新资讯新闻上线的状态
	$phpcmsdb->w('v9_information',array('online_status'=>14), array('id'=>$r['id'])) ;
}
//资讯新闻下线
function __do_publish_to_off_information($r){
	$phpcmsdb = yzy_phpcms_db() ;
	$go3cdb = yzy_go3c_db() ;
	$go3cdb->d('information',array('nid'=>$r['id'])) ;
