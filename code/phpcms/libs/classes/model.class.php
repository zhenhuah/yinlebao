...
class model {
     ...
     * @param $sql    sql语句
     */
	final public function mynum($sql='') {
		$number = $this->db->count_sql($sql);
		return $number;
	}
     ...
     * @param $perpage  每页数量
     */
	final public function mytotalpage($sql='', $perpage = '10') {
		$this->number = $this->db->count_sql($sql);
		$totalpage = ceil($this->number / $perpage);
		return $totalpage;
	}
	...
	final public function myquery($sql) {
	 ...
	 * @return unknown_type
	 */
	final public function listinfo_analysis($where = '', $order = '', $key='', $setpages = 10,$urlrule = '',$array = array()) {
		$where = to_sqls($where);
		$this->number = $this->count($where);
		$array = array();
		if ($this->number > 0) {
